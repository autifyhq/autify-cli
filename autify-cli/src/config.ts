...
type Variable =
  ...
  | "AUTIFY_MOBILE_LINK_SOURCE_URL"
  | "AUTIFY_MOBILE_LINK_STARTUP_TIMEOUT"
  ...
  | "AUTIFY_CONNECT_CLIENT_STARTUP_TIMEOUT"
  | "AUTIFY_CLI_USER_AGENT_SUFFIX"
  | "AUTIFY_TEST_WAIT_INTERVAL_SECOND";

const envFile = (dir: string) => path.resolve(dir, "config.env");
const read = (dir: string) =>
  fs.existsSync(envFile(dir))
    ? envfile.parse(fs.readFileSync(envFile(dir)).toString())
    : {};
const write = (dir: string, config: envfile.Data) =>
  fs.writeFileSync(envFile(dir), envfile.stringify(config));
