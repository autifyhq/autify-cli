...
import {Client as WebClient} from '../../generated/web/client'
import emoji from 'node-emoji'
import {InlineResponse2001StatusEnum} from '../../generated/web/openapi'

const waitUntil = async <T>(callback: (task: ListrTaskWrapper<{result: T}>) => Promise<T>, timeoutSecond: number, intervalSecond: number, verbose: boolean): Promise<T | void> => {
  const task = new Listr<{result: T}>([
    {
      title: `Waiting... (timeout: ${timeoutSecond} s)`,
      task: async (ctx, task) => {
        for await (const startTime of setInterval(intervalSecond * 1000, Date.now())) {
          const now = Date.now()
...
const emojiStatus = (status?: InlineResponse2001StatusEnum) => {
  if (status === 'queuing') return emoji.get('cyclone') + ' Queuing'
  if (status === 'waiting') return emoji.get('hourglass_flowing_sand') + ' Waiting'
...
const describeResult = (client: WebClient, workspaceId: number, resultId: number) => async (task: ListrTaskWrapper) => {
...
export const waitTestResult = async (client: WebClient, workspaceId: number, resultId: number, options: {timeoutSecond: number, verbose: boolean}): Promise<{ isPassed: boolean; data: any }> => {
  const data = await waitUntil(describeResult(client, workspaceId, resultId), options.timeoutSecond, 1, options.verbose)
