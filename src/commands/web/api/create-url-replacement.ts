...
export default class WebApiCreateUrlReplacement extends Command {
  ...
  static flags = {
    ...
    'create-url-replacement-request': Flags.string({description: 'The url to replace', required: true}),
  }
  ...
  public async run(): Promise<void> {
    ...
    const res = await client.createUrlReplacement(flags['test-plan-id'], JSON.parse(flags['create-url-replacement-request']))
