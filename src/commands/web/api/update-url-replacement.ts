...
export default class WebApiUpdateUrlReplacement extends Command {
  ...
  static flags = {
    ...
    'update-url-replacement-request': Flags.string({description: 'The url to replace. Either pattern_url or replacement_url is required.', required: true}),
  }
  ...
  public async run(): Promise<void> {
    ...
    const res = await client.updateUrlReplacement(flags['test-plan-id'], flags['url-replacement-id'], JSON.parse(flags['update-url-replacement-request']))
